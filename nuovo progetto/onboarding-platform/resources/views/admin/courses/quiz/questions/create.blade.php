<x-layout>
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>Aggiungi nuova domanda - {{ $course->title }}</h2>
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Crea domanda</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.courses.quiz.questions.store', $course) }}" method="POST">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                                ...
                                <option value="text" {{ old('type') == 'text' ? 'selected' : '' }}>Risposta testuale</option>
                                <option value="true_false" {{ old('type') == 'true_false' ? 'selected' : '' }}>Vero/Falso</option>
                                <option value="single_choice" {{ old('type') == 'single_choice' ? 'selected' : '' }}>Scelta singola</option>
                                <option value="multiple_choice" {{ old('type') == 'multiple_choice' ? 'selected' : '' }}>Scelta multipla</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="points" id="points" min="1" max="100" class="form-control @error('points') is-invalid @enderror" value="{{ old('points', 1) }}" required>
                            @error('points')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div id="text-answer-section" class="answer-section mb-3" style="display: none;">
                        ...
                        <textarea name="correct_answer" id="correct_answer" rows="2" class="form-control @error('correct_answer') is-invalid @enderror">{{ old('correct_answer') }}</textarea>
                    ...
                    <div id="true_false-answer-section" class="answer-section mb-3" style="display: none;">
                        ...
                        <div class="d-flex">
                            <div class="form-check me-3">
                                <input class="form-check-input" type="radio" name="is_true" id="answer-true" value="1" {{ old('is_true') == '1' ? 'checked' : '' }}>
                                <label class="form-check-label" for="answer-true">Vero</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="is_true" id="answer-false" value="0" {{ old('is_true') == '0' ? 'checked' : '' }}>
                    ...
                    <div id="choice-answer-section" class="answer-section mb-3" style="display: none;">
                        ...
                        <div class="choices-container">
                            @if(old('answers') && (old('type') == 'single_choice' || old('type') == 'multiple_choice'))
